...
function startGame() {
  ...
  game.notes = makeChart(track).map((note, index) => {
    ...
    return { ...note, targetLane, travelMs, isTrash: trashMode() && (index + 1) % 3 === 0, hit: false, missed: false, el: null };
  });
...
function updateModeInfo() { if (trackInfo) trackInfo.textContent = `${randomSpeedMode() ? '랜덤 속도' : '일정 속도'} · ${trashMode() ? '3회마다 쓰레기(-100점)' : '쓰레기 없음'}`; }
